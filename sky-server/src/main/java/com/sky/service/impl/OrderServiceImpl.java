...
//            log.info("申请退款：{}", refund);
    ...
    public OrderPaymentVO payment(OrdersPaymentDTO ordersPaymentDTO) throws Exception {
        // 1. 获取当前登录用户id（其实这一步也可以省，但留着吧）
        Long userId = BaseContext.getCurrentId();
        // User user = userMapper.getById(userId); // 这一行不需要了

        // =========== 核心作弊代码 START ===========

        // 2. 根据订单号查询当前订单
        // 假设你的 Mapper 里有这个方法，如果没有，请用 selectByNumber 或者类似的
        Orders ordersDB = orderMapper.getByNumber(ordersPaymentDTO.getOrderNumber());

        // 3. 强制修改支付状态
        // 设置为“已支付” (通常是 1)
        ordersDB.setPayStatus(Orders.PAID);
        // 设置为“待接单” (通常是 2，即支付成功后等待商家接单)
        ...
        // 5. 返回一个空的 VO 对象
        // 前端收到这个对象后，虽然唤起不了微信支付窗口，但因为我们已经改了数据库状态
        // 你去“我的订单”里刷新一下，就会发现订单已经变成“待接单”了
